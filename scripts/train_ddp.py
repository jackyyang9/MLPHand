...
def main_worker(gpu_id: int, cfg: CN, arg: Namespace, time_f: float):
    ...
    model.module.second_init_for_hypergraph()
        ...
        if (epoch_idx % arg.eval_interval == arg.eval_interval - 1 or epoch_idx == 0) and rank == 0:
            ...
            with torch.no_grad():
                ...
                for bidx, batch in enumerate(valbar):
                    step_idx = epoch_idx * len(val_loader) + bidx # type: ignore
                    preds = model(batch, step_idx, "val")

            model.module.on_val_finished(recorder, epoch_idx) # type: ignore
...
if __name__ == "__main__":
    exp_time = time()
    # sys.argv = ["train_ddp.py", "--cfg", "config/release/MlpHand_DexYCBMV.yaml", "-g", "0", "-w", "4", '-p', '20000']
    ...
    # # for hypergraph
    cfg['TRAIN']['EPOCH']=121
    cfg['MODEL']['TYPE']='MultiviewHandRecon_MLPHand_HyperGraph'
