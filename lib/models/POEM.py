...
from .HM_trainv2 import MLPHand
...
class PtEmbedMultiviewStereo(nn.Module, ModuleAbstract):
    ...
    def _forward_impl(self, batch, **kwargs):
        ...
        # UV解码
        # region ===== uv dec layer >>>>>
        uv_hmap = self.uv_decode(img_feats)  # (BxN, 21, 32, 32), (BxN, 128, 32, 32)
        ...
        # ref_verts = self.MLPhand(batch['master_joints_3d'])
        # ref_verts = self.MLPhand(ref_joints)
        ...
        # preds={}
        # preds["all_coords_preds"]=ref_mesh.unsqueeze(0)
    ...
    def forward(self, inputs, step_idx=None, mode=1, **kwargs):
        if mode == "train":
            ...
            return self._forward_stat2(inputs)
        else:
    ...
    def _forward_stat(self, inputs):
        ...
        # 3D-joint计算，改这里
        K = K  # (B, N, 3, 3)
        T_c2m = torch.linalg.inv(K)  # (B, N, 4, 4)
        ref_joints = batch_triangulate_dlt_torch(uv_coord_im, K, T_c2m)  # (B, J, 3)
    ...
    def _forward_stat2(self, inputs):
        # 输入数据处理
        # 输入数据处理
        img,K,T_c2m,gt_J3d,gt_V3d,cam_intr,cam_extr,master_id=inputs
        img =img# batch["image"]  # (B, N, 3, H, W) 5 dimensions
        batch_size, num_cams = img.size(0), img.size(1)
        inp_img_shape = img.shape[-2:]  # H, W
        H, W = inp_img_shape
        img_feats, img_feats_reshaped, global_feat = self.extract_img_feat(img)  # [(B, N, C, H, W), ...]\
        ...
        # 3D-joint计算，改这里
        K = K#,batch['target_cam_intr']  # (B, N, 3, 3)
        T_c2m = torch.linalg.inv(T_c2m)  # (B, N, 4, 4)
        ref_joints = batch_triangulate_dlt_torch(uv_coord_im, K, T_c2m)  # (B, J, 3)
        ...
        # 开始模型预测
        gt_J3d = gt_J3d#batch["master_joints_3d"]
        gt_V3d = gt_V3d#batch["master_verts_3d"]
        gt_mesh = torch.cat([gt_J3d, gt_V3d], dim=1)  # (B, 799, 3)

        # prepare image_metas
        img_metas = {
            "inp_img_shape": inp_img_shape,  # h, w
            "cam_intr": cam_intr,#batch["target_cam_intr"],  # tensor (B, N, 3, 3)
            "cam_extr": cam_extr,#batch["target_cam_extr"],  # tensor  (B, N, 4, 4)
            "master_id": master_id,#batch["master_id"],
            "ref_mesh_gt": gt_mesh,
            # ...
        }
